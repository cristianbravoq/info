/*
    ...
    A vanilla JS class with more Math functions not included in the JS standard
    library Math object
...
class Maths {
  ...
  static randomInt (min = 1, max) {
    if (typeof max === 'undefined') { max = min; min = 0 }
    return Math.floor(min + Math.random() * (max - min + 1))
  }

  static randomFloat (min = 1, max) {
    if (typeof max === 'undefined') { max = min; min = 0 }
  ...
  static noise (x, y, z, reset) { // via P5.js perlin noise
    if (reset) this.perlin = null
